...
start_url = 'http://drugs.medlive.cn/drugref/drugCateLast.do?treeCode=H0911'
...
def make_url(html):
    ...
    mainurl_0 = html + '&page='
    for i in range(2, 37): # 按实际药品主页页数而定
...
def save(drugname, drug_info):
    ...
    将每一页的网页的源代码保存到本地
    """
    os.makedirs('肝病辅助治疗药（消化系统药物）', exist_ok=True)   # 如果没有"医脉通文件夹，就创建一个，如果有，则什么都不做"（可更改名字）
    x = 0
    for i in drugname:
        with open(os.path.join('肝病辅助治疗药（消化系统药物）', i + '.txt'), 'w', encoding='utf-8') as f:  # 名字可更改
            f.write(drug_info[x])
            x = x+1
...
if __name__ == '__main__':
    ...
    # page_link_list = page_link_list[2:]  # 第一次爬的时候不要用，中断之后按爬了多少页进行切片继续
