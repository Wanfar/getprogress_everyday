...
start_url = 'https://www.booktxt.net/3_3482/'
...
def get_toc(html):
    ...
    """
    toc_url_list = []
    toc_block = re.findall('正文(.*?)</dl>', html, re.S)[0]
...
def get_article(html):
    ...
    :return: 章节名，正文
    """
    chapter_name = re.search('<title>(.*?)_', html, re.S).group(1)
    text_block = re.search('&nbsp;&nbsp;&nbsp;&nbsp;(.*?)</div>', html, re.S).group(1)
    text_block = text_block.replace('<br /><br />', '\n')
...
def save(chapter, article):
    ...
    with open(os.path.join('微微一笑很倾城', chapter + '.txt'), 'w', encoding='utf-8') as f:
        f.write(article)
